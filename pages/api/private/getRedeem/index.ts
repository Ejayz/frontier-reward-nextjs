  ...
  try {
    ...
    [rows] = <RowDataPacket[]>(
      await connection.query(
        `select *,redeem.name as redeem_name,redeem.description as redeem_description,reward.name as reward_name, packages.name as package_name from redeem LEFT JOIN reward ON reward.id = redeem.reward_id LEFT JOIN packages ON packages.id = redeem.package_id where redeem.is_exist=1 limit ? , ?`,
        [offset, limit]
      )
    );
